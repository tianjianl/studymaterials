...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.encoder = nn.LSTM(word_embed_size, hidden_size, num_layers=1, bidirectional=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, (last_hidden, last_cell) = self.encoder.forward(X)
        enc_hiddens, lens_unpacked = torch.nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        init_decoder_hidden = self.h_projection.forward(torch.cat((last_hidden[0, :, :], last_hidden[1, :, :]),-1))
        init_decoder_cell = self.c_projection.forward(torch.cat((last_cell[0, :, :], last_cell[1, :, :]), -1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings_target.forward(target_padded)
        for Y_t in torch.split(Y, 1, dim=0):
            #Y_t is shape of (1, b, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #now alpha_t shape is (b, 1, s)
        a_t = torch.bmm(alpha_t, enc_hiddens)
        #now a_t shape is (b, 1, 2h)
