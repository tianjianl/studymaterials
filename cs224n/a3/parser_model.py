...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        nn.init.xavier_uniform_(w)
        b = torch.empty((1, self.n_classes))
        nn.init.uniform_(b)
    ...
    def embedding_lookup(self, w):
        ...
        for i in range(batch_size):
            for index, feature in enumerate(w[i]):
                feature = torch.index_select(self.embeddings, 0, feature)
                x[i, index, :] = feature
                
        #shape of x should be (B, N*embed_size)
