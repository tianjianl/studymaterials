...
def naiveSoftmaxLossAndGradient(
    ...
    y[outsideWordIdx] += 1 #one hot encoding of the outside vector
...
def negSamplingLossAndGradient(
    ...
    for i in negSampleWordIndices:
        sigmoid_uk_vc = sigmoid(np.dot(-outsideVectors[i], centerWordVec))
        loss += -np.log(sigmoid_uk_vc)
        gradCenterVec += (1 - sigmoid_uk_vc) * outsideVectors[i]
        gradOutsideVecs[i] += (1 - sigmoid_uk_vc) * centerWordVec 
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        nowloss, gradcenter, gradoutside = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[word], outsideVectors, dataset)
        loss += nowloss
